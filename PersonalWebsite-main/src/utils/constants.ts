// export const NavLinks = [
//   { link: "#about", value: "About" },
//   { link: "#work", value: "Work" },
//   { link: "#projects", value: "Projects" },
//   { link: "#contact", value: "Contact Me" },
// ];


export const AboutList = [
  {
    topic: "About me",
    role: "About me",
    bullets: ["My name is Ahmed, and I'm a software engineer with a strong passion for learning new technologies and working on impactful projects.",
      "Aside from software engineering, I enjoy reading, playing board games, and spending time with family and friends.",
      "Reach out to talk about any of these topics :)"
    ],
    
  },
  {
    topic: "Skills",
    role: "My skills",
    bullets: ["• Languages: C/C++, C#, Python, HTML/CSS, JS/TS, PHP, SQL, Java, ARM Assembly.",
      "• Frameworks & Libraries: React, Express, Tailwind, Bootstrap, Expo, Django, Django REST, ASP.NET, .NET Framework, WPF. ",
      "• Platforms & Tools: Git/GitHub, Shell, Azure, MySQL, PostgreSQL, Azure DevOps, Postman, IIS, Linux, Selenium, Lucidchart, Figma, Logisim, CodeWarrior, NodeJS, Microsoft 365.",
    ],
  },
  {
    topic: "Education",
    role: "Education",
    bullets: ["Conestoga College - Waterloo, ON⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀Sep 2023 - Apr 2026 ⠀⠀• Software Engineering grad with a 3.80/4.00 GPA. ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ ⠀⠀• Member of Google Developer Student Club.",
      "Eastowood Colligate Institute - Kitchener, ON ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀Sep 2019 - Jun 2023 ⠀⠀• Graduated with a grade 12 average of 86% ⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀ ⠀⠀• Played soccer throughout."
    ],
    mobileBullets: [
      "Conestoga College: Currently in 2nd year of Software Engineering with a 3.83/4.00 GPA • Member of Google Developer Student Club.",
      "Eastwood Collegiate Institute: Graduated with a Gr. 12 average of 86% • Played soccer throughout my time."
    ],
    
  },
];

export const ExperienceList = [
  // {
  //   company: "Incoming",
  //   url: "",
  //   role: "Software Developer",
  //   timeline: "Sep 2025 - ",
  //   bullets: ["Incoming..."],
  //   location: "",
  // },
  {
    company: "ENK Maintenance",
    url: "",
    role: "Cleaning Specialist",
    timeline: "Jun 2023 - May 2025",
    bullets: ["Performed cleaning duties of residential and commercial spaces, including vacuuming, dusting, wiping, and floor sanitation to maintain high cleanliness standard.", 
      "Managed time efficiently to complete multiple cleaning tasks within a scheduled timeframe.",
      "Monitored inventory of cleaning supplies to ensure continuous operation.",
      "Worked independently and as part of a team to meet client expectations.",
      "Followed supervisor instructions to ensure consistent, high-quality results."],
    location: "Kitchener, ON",
  },
];

export const ProjectList = [
  // {
  //   name: "Co-opoly",
  //   skills: [
  //     "Unity",
  //   ],
  //   timeline: "WORK IN PROGRESS...",
  //   text: "A coop version of the beloved board-game classic Monopoly. You get to team up with your friends and compete against rival teams - sharing wealth, resources, and victory in the process.",
  //   url: "",
  // },
  {
    name: "MetaBuild",
    skills: [
      "TypeScript",
      "Python",
      "NextJS",
      "Django REST",
      "Bootstrap",
      "PostgreSQL",
      "Git",
    ],
    timeline: "Work in Progress...",
    text: "An intelligent PC builder tool that allows users to generate the best possible computer builds based on their budget, use case, and preferences.",
    url: "https://github.com/ahmedalmoune/MetaBuild",
  },
  {
    name: "SquadQuest",
    skills: [
      "Expo",
      "TensorFlow.js",
      "MoveNet",
      "Supabase",
      "TypeScript",
      "Git",
    ],
    timeline: "April 2026",
    text: "A group-based goal tracking app that uses AI verification and gamification to help users complete goals and build habits through social accountability and collaborative progress tracking.",
    url: "https://setprojectday.ca/post/2026/squadquest/",
  },
  // {
  //   name: "MyGallery",
  //   skills: [
  //     "Python",
  //     "Django",
  //     "JavaScript",
  //     "Bootstrap",
  //     "Git",
  //   ],
  //   timeline: "Jan 2026",
  //   text: "A gallery/photo sharing web app for people to share their photos and interact with other users.",
  //   url: "https://github.com/ahmedalmoune/Gallery",
  // },
  // {
  //   name: "Dev Web Server",
  //   skills: [
  //     "C#",
  //     "Postman",
  //     "TCP/IP",
  //     "Git",
  //   ],
  //   timeline: "Nov 2025",
  //   text: "A lightweight web server for developers to test their network applications during development.",
  //   url: "https://github.com/ahmedalmoune/Web-Server",
  // },
    {
    name: "RealiableUDP (Academic)",
    skills: [
      "C++",
      "UDP",
      "MD5",
      "Git",
    ],
    timeline: "Dec 2025",
    text: "An implementation of UDP made reliable using packet sequencing, acknowledgment, retransmission, and MD5 checksums to ensure data integrity and order.",
    url: "https://github.com/ahmedalmoune/ReliableUDP",
  },

];
